...
const safeAssetName = z
  ...
  .refine((value) => !value.includes('..'), 'Asset names cannot contain parent-directory segments.');
...
export const mediaReleaseAssetSchema = z
  ...
  .superRefine((asset, context) => {
    ...
    for (const field of ['immutableUrl', 'githubDigest', 'verifiedBytes', 'verifiedAt'] as const) {
      if (asset[field] === undefined) {
        context.addIssue({ code: 'custom', path: [field], message: `${field} is required for a published asset.` });
      }
    }
    if (asset.verifiedBytes !== undefined && asset.verifiedBytes !== asset.expectedBytes) {
      context.addIssue({ code: 'custom', path: ['verifiedBytes'], message: 'Verified bytes must match expectedBytes.' });
    }
    if (asset.githubDigest !== undefined && asset.githubDigest !== asset.expectedSha256) {
      context.addIssue({ code: 'custom', path: ['githubDigest'], message: 'GitHub digest must match expectedSha256.' });
    }
  });
...
export const mediaReleaseVolumeSchema = z
  .object({
    ...
    expectedAssetCount: z.number().int().min(1).max(MAX_MEDIA_ASSETS_PER_RELEASE),
    publicationState: z.enum(['planned', 'uploading', 'published']),
    assets: z.array(mediaReleaseAssetSchema).min(1).max(MAX_MEDIA_ASSETS_PER_RELEASE),
  ...
  .superRefine((volume, context) => {
    if (volume.assets.length !== volume.expectedAssetCount) {
      context.addIssue({ code: 'custom', path: ['expectedAssetCount'], message: 'expectedAssetCount must equal the tracked asset count.' });
    }
    if (volume.publicationState === 'published' && volume.assets.some((asset) => asset.assetState !== 'published')) {
      context.addIssue({ code: 'custom', path: ['publicationState'], message: 'A published volume cannot contain an incomplete asset.' });
    }
    for (const [index, asset] of volume.assets.entries()) {
      if (volume.publicationState === 'published' && asset.assetState !== 'published') {
        context.addIssue({ code: 'custom', path: ['assets', index, 'assetState'], message: 'Published volumes require published assets.' });
      }
    }
  });
  ...
  .superRefine((registry, context) => {
    ...
    for (const [releaseIndex, release] of registry.releases.entries()) {
      if (releaseTags.has(release.releaseTag)) {
        context.addIssue({ code: 'custom', path: ['releases', releaseIndex, 'releaseTag'], message: 'Release tags must be unique.' });
      }
      ...
      if (release.releaseUrl !== expectedReleaseUrl) {
        context.addIssue({ code: 'custom', path: ['releases', releaseIndex, 'releaseUrl'], message: 'Release URL must address the exact immutable release tag.' });
      }
      for (const [assetIndex, asset] of release.assets.entries()) {
        if (mediaIds.has(asset.mediaId)) context.addIssue({ code: 'custom', path: ['releases', releaseIndex, 'assets', assetIndex, 'mediaId'], message: 'Media IDs must be unique.' });
        if (titles.has(asset.canonicalTitle)) context.addIssue({ code: 'custom', path: ['releases', releaseIndex, 'assets', assetIndex, 'canonicalTitle'], message: 'Canonical media titles must be unique.' });
        if (assetNames.has(asset.releaseAssetName)) context.addIssue({ code: 'custom', path: ['releases', releaseIndex, 'assets', assetIndex, 'releaseAssetName'], message: 'Release asset names must be unique.' });
        ...
        if (asset.assetState === 'published') {
          ...
          if (asset.immutableUrl !== expectedAssetUrl) {
            context.addIssue({ code: 'custom', path: ['releases', releaseIndex, 'assets', assetIndex, 'immutableUrl'], message: 'Published assets must use the exact GitHub release download URL.' });
          }
        }
      }
    }

    if (registry.registryState === 'empty' && (registry.releases.length !== 0 || publishedAssets !== 0)) {
      context.addIssue({ code: 'custom', path: ['registryState'], message: 'An empty registry cannot contain releases or published assets.' });
    }
    if (registry.registryState === 'contains-published-assets' && publishedAssets === 0) {
      context.addIssue({ code: 'custom', path: ['registryState'], message: 'A non-empty registry requires at least one published asset.' });
    }
  });
...
export const mediaReleaseRegistry = mediaReleaseRegistrySchema.parse(rawRegistry) as MediaReleaseRegistryV1;

/** Exact-title lookup for the reader. Unverified or non-published records are never returned. */
export function publishedMediaAssetForTitle(title: string): MediaReleaseAssetV1 | null {
  for (const release of mediaReleaseRegistry.releases) {
    if (release.publicationState !== 'published') continue;
    const asset = release.assets.find((candidate) => candidate.canonicalTitle === title);
    if (!asset || asset.assetState !== 'published' || !asset.immutableUrl) continue;
