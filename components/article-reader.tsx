...
function articleHtmlForSurface(html: string) {
  return html
    .replace(
      /href="(\/wiki\/[^\"]*)"/g,
      (_, pathname: string) => `href="${publicPath(pathname)}"`,
    )
...
function displayLabel(
  id: string,
  fallback: string,
  languageMode: 'en' | 'zh-HK' | 'bilingual',
) {
...
function articlePresentation(
  ...
  if (
    /^\d{4}$/.test(record.displayTitle.trim()) ||
    record.categories.some(
      (category) => category.toLocaleLowerCase() === 'years',
    )
  ) {
    return 'year';
  }
  if (
    record.displayTitle.toLocaleLowerCase().endsWith(' station') ||
    record.categories.some((category) =>
      category.toLocaleLowerCase().includes('station'),
    )
  ) {
    return 'station';
  }
...
function DeferredMediaNotice({ titles }: { titles: string[] }) {
  if (!titles.length)
    return <p>No source media file titles were referenced in this article.</p>;
  return (
    <p>
      Media deferred:{' '}
...
export function ArticleReader({
  ...
  const [activeSection, setActiveSection] = useState(
    record.headings[0]?.id ?? 'article-content',
  );
  ...
  return (
    <div
      className="article-app-shell"
      data-element-id="article-shell"
    ...
    >
      ...
      <header
        className="top-bar article-app-bar"
        aria-label="Nazca Railway header"
      >
        <button
          className="brand"
          type="button"
          onClick={() => navigateDestination('home')}
      ...
      <div className="route-color-bar" aria-hidden="true">
        <span />
        <span />
        <span />
        <span />
      ...
      <div className="article-shell-grid">
        <nav
          className="tab-dock article-tab-dock"
                ...
                {group.items.map((item) => {
                  ...
                  return (
                    <button
                      ...
                      title={displayLabel(item.id, item.label, languageMode)}
                      onClick={() => navigateDestination(item.id)}
                    >
                      <span className="tab-icon">
                        <Icon size={18} aria-hidden="true" />
                      </span>
                      <span>
                        {displayLabel(item.id, item.label, languageMode)}
                      </span>
                      {item.count ? (
        ...
        <main className="article-main-viewport" id="main-content" tabIndex={-1}>
          <div className="article-layout article-layout-reference">
            <article
              className={`article-body article-body-${presentation}`}
              id="article-content"
              tabIndex={-1}
              data-reader-state={`article-${presentation}`}
            >
              ...
              <p className="eyebrow article-eyebrow">
                ...
                {presentation === 'station'
                  ? 'STATION · CURRENT SNAPSHOT'
                  : presentation === 'year'
                    ? 'YEAR · CURRENT SNAPSHOT'
                    : 'CURRENT ARTICLE · SOURCE SNAPSHOT'}
              </p>
              <h1
                className={
                  presentation === 'year' ? 'article-year-heading' : undefined
                }
              >
                {record.displayTitle}
              </h1>
              <p className="article-subtitle">
                {presentation === 'station'
                  ? 'A captured station record from the current reader index.'
                  : presentation === 'year'
                    ? 'A captured year record from the current reader index.'
                    : 'Current article body captured from the source.'}{' '}
                Revision <code>{record.currentRevisionId}</code>. This reader is
                a static, sanitized presentation.
              </p>
              {presentation !== 'generic' ? (
                <section
                  className="reader-fact-grid"
                  aria-label={
                    presentation === 'station'
                      ? 'Station record facts'
                      : 'Year record facts'
                  }
                >
                  <div className="reader-fact-card reader-fact-card-primary">
                    <span className="reader-fact-label">
                      {presentation === 'station'
                        ? 'STATION RECORD'
                        : 'YEAR RECORD'}
                    </span>
                    <strong>{record.displayTitle}</strong>
                    <span>
                      {record.plainTextExcerpt.slice(0, 220)}
                      {record.plainTextExcerpt.length > 220 ? '…' : ''}
                    </span>
                  </div>
                  ...
                  <div className="reader-fact-card">
                    ...
                    <span>
                      {record.deferredMedia.length
                        ? `${record.deferredMedia.length} media references deferred`
                        : 'No media references deferred'}
                    </span>
                  </div>
                </section>
              ) : null}
              <div
                className="article-meta-tabs"
                role="tablist"
              ...
              >
                ...
                <a href="#history">
                  <History size={15} aria-hidden="true" /> History boundary
                </a>
              </div>
              <section id="read" className="article-section">
                {presentation !== 'generic' ? (
                  <h2 className="reader-captured-heading">
                ...
                <div
                  ...
                  dangerouslySetInnerHTML={{
                    __html: articleHtmlForSurface(record.safeHtml),
                  }}
                />
              </section>
                ...
                <div>
                  ...
                  <p>
                    ...
                    <a
                      ...
                      referrerPolicy="no-referrer"
                    >
                      Fandom revision {record.currentRevisionId}
                    </a>
                    . Text is presented under the source CC BY-SA terms.
                  </p>
                  <p>
                    Revision timestamp: {record.timestamp}. Contributor:{' '}
                    {record.contributorState === 'visible'
                      ? record.contributor
                      : 'hidden or unavailable'}
                    .
                  </p>
                  <p>
                    Historical revisions, maps, and media bytes remain outside
              ...
              <section id="history" className="article-source-card">
                <div>
                  ...
                  <p>
                    This release includes the current revision only. Complete
                    revision history is explicitly deferred.
                  </p>
            ...
            <aside className="article-right-rail" aria-label="Article summary">
              <section className="article-summary-card">
                ...
                <dl>
                  <div>
                    <dt>Title</dt>
                    <dd>{record.displayTitle}</dd>
                  </div>
                  <div>
                    <dt>Sections</dt>
                    <dd>{record.headings.length}</dd>
                  </div>
                  <div>
                    <dt>Revision</dt>
                    <dd>{record.currentRevisionId}</dd>
                  </div>
                  <div>
                    <dt>Captured</dt>
                  ...
                  <div>
                    ...
                    <dd>
                      {record.deferredMedia.length
                        ? `${record.deferredMedia.length} deferred`
                        : 'None referenced'}
                    </dd>
                  </div>
                </dl>
              ...
              <section className="article-section-card">
                ...
                <nav
                  className="article-section-links"
                  aria-label="Article section links"
                >
                  {record.headings.slice(0, 18).map((heading) => (
                    <button
                      ...
                      type="button"
                      aria-current={
                        heading.id === activeSection ? 'page' : undefined
                      }
                      onClick={() => openSection(heading.id)}
                    >
                      <span>
                        {heading.heading.replace(/\s*\[\s*\]\s*$/, '')}
                      </span>
                      <span aria-hidden="true">›</span>
                    </button>
                  ))}
      ...
      <nav className="reader-bottom-nav" aria-label="Reader quick navigation">
        <button type="button" onClick={() => navigateDestination('home')}>
          <Home size={19} aria-hidden="true" />
          <span>Home</span>
        </button>
        <button type="button" onClick={() => navigateDestination('stations')}>
        ...
        <button
          type="button"
          onClick={() =>
            document.getElementById('article-global-search-input')?.focus()
          }
        >
          <Search size={19} aria-hidden="true" />
          <span>Search</span>
        </button>
        <button type="button" onClick={focusArticle}>
